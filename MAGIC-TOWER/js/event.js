    ...
    if(objectNum<100&&objectNum>0){
        ...
        if(hero.att>monText.def){//角色攻击大于怪物
            ...
            if(hero.HP>damage){//角色血量大于造成伤害
                ...
                if(hero.x==6&&hero.y==3){
                    setTimeout(function(){bosstalktwo()},700);
                    setTimeout(function(){victory()},700);
                }
            }
            else{
                hero.x=hero.x*2-y-1;
...
$(document).ready(function(){       //各类点击事件
    $("#monsterBook").click(function(){         //怪物书点击事件
        ...
        if(hero.book==1){
            ...
            $("#book").show();
            $("#book").animate({opacity:'0.8'});
            $("#cancel").show();
            $("#cancel").animate({opacity:'0.8'});
            $("#book ul").remove();
            $("#book").append("<ul></ul>");
            appendMonster();
        }
    ...
    $("#cancel").click(function(){
        $("#book").animate({opacity:'0'});
        $("#cancel").animate({opacity:'0'});
        setTimeout(function(){$("#book").hide()},300);
        setTimeout(function(){$("#cancel").hide()},300);
        hero.enable=1;
    });
    ...
    $("#talk div:nth-child(2)").click(function (){
        if(hero.money<hero.buyTimes*hero.buyTimes*10+20-10*hero.buyTimes) {
            alertify.alert("您的钱不够！")
            return;
        }
    ...
    $("#talk div:nth-child(5)").click(function (){
        $("#talk").animate({opacity:'0'});
        setTimeout(function(){$("#talk").hide()},300);
        hero.enable=1;
    });
});
...
victory=()=>{
    setObject(hero.floor,2,0,207);
    setObject(hero.floor,2,1,203);
    setObject(hero.floor,2,2,207);
    setObject(hero.floor,2,8,208);
    setObject(hero.floor,2,9,204);
    setObject(hero.floor,2,10,208);
    setObject(hero.floor,3,0,206);
    setObject(hero.floor,3,2,206);
    setObject(hero.floor,3,8,205);
    setObject(hero.floor,3,10,205);
}